 ...
 *
 * WASM Export Map (jitsi/rnnoise-wasm minified build):
 *   c = memory           d = __wasm_call_ctors   e = rnnoise_init (per-state, do NOT call directly)
...
class NoiseSuppressorProcessor extends AudioWorkletProcessor {
  ...
  async _initializeFromBinary(wasmBinary) {
    try {
      ...
      // IMPORTANT: Do NOT call rnnoise_init (export "e") here.
      // rnnoise_init is a per-state initializer — rnnoise_create (export "f") calls
      // it internally with a properly allocated state pointer.
      // Calling rnnoise_init() with no args passes NULL (0) as the state pointer,
      // which executes memset(0, 0, sizeof(DenoiseState)), zeroing ~87KB starting at
      // WASM address 0. This corrupts the neural network model weights that
      // __wasm_call_ctors just placed in static memory, causing RNNoise to run but
      // produce no suppression (vad≈0, reduction≈0dB instead of -40 to -80dB).
